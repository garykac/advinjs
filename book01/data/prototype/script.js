...
function init_game() {
	...
	_game.font_family = "archisticobold";
	...
	// Status images
	_game.img_win = new Image();
	_game.img_win.src = _game.imagedir_items + "gem.png";
...
function draw() {
	...
	if (_game.game_over) {
		...
		ctx.fillStyle = "black";
		ctx.font = "48px " + _game.font_family;
		if (_game.game_win) {
			ctx.fillText("You Win!", 155, 150);
			ctx.drawImage(_game.img_win, 265, 170);
		} else {
...
function draw_goal(ctx) {
	...
	if (goal) {
		...
		if (_game.debug) {
			ctx.strokeStyle = "gray";
			ctx.strokeRect(0, 0, goal.width, goal.height);
		}
		reset_transform(ctx);
	}
}
...
function draw_player(ctx) {
	...
	if (_game.debug) {
		ctx.strokeStyle = "gray";
		ctx.strokeRect(0, 0, _player.width, _player.height);
	}
	reset_transform(ctx);
}
